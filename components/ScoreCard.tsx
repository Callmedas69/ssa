...
const providerInfo: Record<
  ScoreProvider,
  { name: string; description: string; maxScore: string }
> = {
  neynar: {
    name: "Neynar",
...
function getExtraInfo(provider: ScoreProvider, data: ScoreData): string | null {
  ...
  if (provider === "ethos") {
    ...
    return ethos.level
      ? ethos.level.charAt(0).toUpperCase() + ethos.level.slice(1)
      : null;
  }
  if (provider === "talentBuilder") {
...
export const ScoreCard = memo(function ScoreCard({
  ...
  const scoreDisplay = useMemo(
    () => getScoreDisplay(provider, data),
    [provider, data]
  );
  const percentage = useMemo(
    () => getScorePercentage(provider, data),
    [provider, data]
  );
  const extraInfo = useMemo(
    () => getExtraInfo(provider, data),
    [provider, data]
  );
